...
function drawBlock(block, idx, x, y, id) {
    ...
    hash_box.y = 200;
    hash_box.graphics.beginFill("#444444").rect(0, 0, BLOCK_WIDTH, 50);
    const previous_hash_label = new createjs.Text("Previous hash: ", font_face, font_color2);
    previous_hash_label.y = 200;
...
function drawBlocks() {
    BLOCKS.map((block, idx) => {
        var rect = drawBlock(block, idx, 200*idx, Y, 0);
        RECTS.push(rect);
    })

    BLOCKS.map((block, idx) => {
        var rect = drawBlock(block, idx, 200*idx, Y+300, 1);
        RECTS.push(rect);
    })

    BLOCKS.map((block, idx) => {
        var rect = drawBlock(block, idx, 200*idx, Y+600, 2);
        RECTS.push(rect);
    })
